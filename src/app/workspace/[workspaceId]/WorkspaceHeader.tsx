...
const WorkspaceHeader = ({ workspace, isAdmin }: WorkspaceHeaderProps) => {
  ...
  return (
    <>
    <PreferencesModal open={preferencesOpen} onOpenChange={setPreferencesOpen} initialValue={workspace.name}/>
    <div className="flex items-center justify-between px-4 h-[49px] gap-0.5 w-full min-w-0">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="transparent"
            className="font-semibold text-lg p-1.5 min-w-0 flex-1 max-w-max text-left"
            size="sm"
          >
            <span className="text-left font-bold truncate flex-1 capitalize">{workspace.name}</span>
